...
class DockDetector(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('a_P', 0.9)
        self.declare_parameter('a_D', 0.05)
        self.declare_parameter('l_P', 0.9)
        self.declare_parameter('l_D', 0.9)
        self.declare_parameter('foview', 360.0)
        self.getting_params()
        ...
        self.linear_controller = Controller(P=self.l_P, D=self.l_D)
    ...
    def getting_params(self):
        ...
        self.a_P = self.get_parameter(
            'a_P').get_parameter_value().double_value

        self.a_D = self.get_parameter(
            'a_D').get_parameter_value().double_value

        self.l_P = self.get_parameter(
            'l_P').get_parameter_value().double_value

        self.l_D = self.get_parameter(
            'l_D').get_parameter_value().double_value

        self.fov = self.get_parameter(
            'foview').get_parameter_value().double_value


        self.get_logger().info("## Dock Charger Manager for robot_name ==="+str(self.robot_name))
        self.get_logger().info("## Field of View (FOV) of the robot's lidar ===" +
                               str(self.fov ))
