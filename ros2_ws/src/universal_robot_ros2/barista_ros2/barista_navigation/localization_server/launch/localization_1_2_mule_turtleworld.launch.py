...
def generate_launch_description():
    ...
    barista_2_config = os.path.join(get_package_share_directory(
        'localization_server'), 'config', 'barista_2_amcl_config_turtleworld.yaml')

    mule_1_config = os.path.join(get_package_share_directory(
        'localization_server'), 'config', 'mule_1_amcl_config_turtleworld.yaml')
    ...
    return LaunchDescription([
        ...
        Node(
            ...
            parameters=[{'use_sim_time': True},
                        {'autostart': True},
                        {'bond_timeout': 0.0},
                        {'node_names': ['map_server', 'barista_1/amcl', 'barista_2/amcl', 'mule_1/amcl']}]
        )
    ])
